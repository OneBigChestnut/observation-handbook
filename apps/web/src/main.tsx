...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {activeNav === "导出文件" && <section className="export-view">{exportNotice && <div className="export-success"><span>✓</span>{exportNotice}<button aria-label="关闭提示" onClick={() => setExportNotice("")}>×</button></div>}<div className="export-launch"><div><b>将观察手册导出为 PDF</b><p>请使用右上角“＋ 导出手册”选择观察手册和 PDF 类型；屏幕版无出血和裁切线，印刷版则会执行预检。</p></div></div><div className="export-rule"><p>已生成文件</p><i></i><span>{exportFiles.length} 个文件</span></div><div className="export-list">{exportFiles.map(file => <ExportRow key={file.id} file={file} onDelete={id => setExportFiles(current => removeGeneratedExport(current, id))} onDownload={downloadExport} />)}</div></section>}
