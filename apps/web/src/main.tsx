  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {isRecordView && <section className="month-view">
        ...
        <ChildContentLoader childId={currentChildId} view={view} />
      </section>}
      {activeNav === "观察手册" && <section className="handbook-view">{handbookNotice && <div className="export-success"><span>✓</span>{handbookNotice}<button aria-label="关闭提示" onClick={() => setHandbookNotice("")}>×</button></div>}{selectedHandbookDetail ? <><div className="admin-rule"><p>观察手册详情</p><i></i><button onClick={() => setSelectedHandbookDetail(null)}>← 返回手册</button></div><article className="handbook-detail"><img src={imageUrl(selectedHandbookDetail.cover, 900)} alt="" /><div><span>{selectedHandbookDetail.publishedAt ? "已公开" : "家庭内"}</span><h2>{selectedHandbookDetail.title}</h2><p>{selectedHandbookDetail.introduction}</p><dl><div><dt>时间</dt><dd>{selectedHandbookDetail.startedAt}{selectedHandbookDetail.completedAt ? ` — ${selectedHandbookDetail.completedAt}` : " — 至今"}</dd></div><div><dt>规格</dt><dd>A5 竖版</dd></div><div><dt>卡片</dt><dd>{handbookDetailCards.length} 张已归入记录</dd></div></dl><div className="handbook-detail-actions"><button className="new-card secondary" onClick={() => openHandbookEditor(selectedHandbookDetail)}>编辑手册</button><button className="new-card" onClick={() => openHandbookExport(selectedHandbookDetail)}>导出 PDF</button><button className="new-card secondary" onClick={() => toggleHandbookPublication(selectedHandbookDetail)}>{selectedHandbookDetail.publishedAt ? "撤回公开" : "发布公开"}</button></div></div></article><div className="handbook-rule detail-rule"><p>收录卡片</p><i></i><span>{handbookDetailCards.length} 张缩略记录</span></div><div className="card-grid">{handbookDetailCards.map(card => <CardTile key={card.id} card={card} />)}</div></> : <><div className="handbook-rule"><p>正在整理</p><i></i><span>按最近更新</span></div><div className="handbook-grid">{handbookItems.map(handbook => <HandbookTile key={handbook.id} handbook={handbook} onTogglePublication={toggleHandbookPublication} onOpen={setSelectedHandbookDetail} />)}</div></>}</section>}
