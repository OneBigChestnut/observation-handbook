...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {isHandbookDialogOpen && <div className="composer-backdrop" role="presentation" onMouseDown={() => setHandbookDialogOpen(false)}><form className="export-dialog" onSubmit={event => { event.preventDefault(); setHandbookDialogOpen(false); }} onMouseDown={event => event.stopPropagation()}><header><div><p>新建观察手册</p><h2>选择装帧模板</h2></div><button type="button" aria-label="关闭新建手册" onClick={() => setHandbookDialogOpen(false)}>×</button></header><label className="field-label" htmlFor="cover-template">封面模板</label><select id="cover-template" value={selectedCoverTemplate} onChange={event => setSelectedCoverTemplate(event.target.value)}><option>A4 竖版 · 自然封面</option><option>A5 竖版 · 档案封面</option></select><label className="field-label" htmlFor="back-template">封底模板</label><select id="back-template" value={selectedBackTemplate} onChange={event => setSelectedBackTemplate(event.target.value)}><option>A4 竖版 · 标准封底</option><option>A5 竖版 · 简洁封底</option></select><p className="dialog-note">封面会显示小朋友姓名、手册名称与发布时间。</p><footer><button type="button" onClick={() => setHandbookDialogOpen(false)}>取消</button><button className="save-card" type="submit">继续创建</button></footer></form></div>}
    </main>
  </div>;
}
