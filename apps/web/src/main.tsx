  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <section className="page-heading">
        <div><p className="eyebrow">{child}的观察档案 · 2026</p><h1>{heading}</h1>{activeNav !== "导出文件" && <p className="subhead">{isRecordView ? "把当下的发现，收进时间的册页。" : activeNav === "观察手册" ? "将同一主题的发现编成可以持续生长的手册。" : activeNav === "标签管理" ? "为观察命名，并把同一主题的记录聚拢在一起。" : "按孩子独立整理和查看内容。"}</p>}</div>
      ...
      {activeNav === "导出文件" && <section className="export-view">{exportNotice && <div className="export-success"><span>✓</span>{exportNotice}<button aria-label="关闭提示" onClick={() => setExportNotice("")}>×</button></div>}<div className="export-launch"><p>生成后可下载或删除；屏幕版无出血，印刷版含 3mm 出血与裁切线。</p></div><div className="export-rule"><p>已生成文件</p><i></i><span>{exportFiles.length} 个文件</span></div><div className="export-list">{exportFiles.map(file => <ExportRow key={file.id} file={file} onDelete={id => setExportFiles(current => removeGeneratedExport(current, id))} onDownload={downloadExport} />)}</div></section>}
