...
describe("handbook api", () => {
  it("prefills a handbook from its tag without automatically adding later matching cards", async () => {
    ...
    const published = await app.inject({ method: "POST", url: `/api/handbooks/${created.json().handbook.id}/publish`, headers: { cookie } });
    ...
    expect(listed.json()).toMatchObject({ handbooks: [{ title: "银杏的一年", cardCount: 1, tagCount: 1, publication: { id: published.json().publication.id } }] });
